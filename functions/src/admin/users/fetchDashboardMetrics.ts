 ...
 */
import { BookingViewModel } from "../../../../services"; // Import BookingViewModel and DashboardMetricsRequest
...
export const fetchDashboardMetrics = onCall(
    ...
    async (req: any) => {
        ...
        const dashboardMetricsRequest = req.data as any;
        ...
        bookings = bookings.docs.map((doc: any) => ({
            id: doc.id,
            ...doc.data(),
        })) as BookingViewModel[];
        ...
        // get client names
        const clients = await firestore.collection(Collection.PROFILES).where("id", "in", bookings.map((booking: any) => booking.clientId)).get();
        ...
        bookings = bookings.map((booking: any) => ({
            ...booking,
            clientName: clientNames.find((client: any) => client.id === booking.clientId)?.name
        }));

        // Upcoming bookings
        // where bookingDate is greater than or equal to today
        const upcomingBookings = bookings.filter((booking: any) => new Date(booking.bookingDate._seconds * 1000) >= new Date());
        ...
        const lastThirtyDaysBookings = bookings.filter((booking: any) => {
